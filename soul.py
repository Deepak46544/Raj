...
def send_not_approved_message(chat_id):
    bot.send_message(chat_id, "*YOU ARE NOT APPROVED*", parse_mode='@raj14754')
...
def approve_user(message):
    if not is_user_admin(message.from_user.id, message.chat.id):
        bot.send_message(message.chat.id, "*You are not authorized to use this command*", parse_mode='@raj14754')
        return

    try:
        cmd_parts = message.text.split()
        if len(cmd_parts) != 4:
            bot.send_message(message.chat.id, "*Invalid command format. Use /approve <user_id> <plan> <days>*", parse_mode='@raj14754')
            return
        ...
        valid_until = (datetime.now() + timedelta(days=days)).date().isoformat() if days > 0 else "@raj14754"
        users_collection.update_one(
            {"user_id": target_user_id},
            {"$set": {"plan": plan, "valid_until": valid_until, "access_count": 0}},
            upsert=True
        )
        bot.send_message(message.chat.id, f"*User {target_user_id} approved with plan {plan} for {days} days.*", parse_mode='@raj14754')
    except Exception as e:
        bot.send_message(message.chat.id, "*shi se add kro na *", parse_mode='@raj14754')
...
def disapprove_user(message):
    ...
    try:
        cmd_parts = message.text.split()
        if len(cmd_parts) != 2:
            bot.send_message(message.chat.id, "*Invalid command format. Use /disapprove <user_id>*", parse_mode='@raj14754')
            return
        ...
        bot.send_message(message.chat.id, f"*User {target_user_id} disapproved and reverted to free.*", parse_mode='@raj14754')
    except Exception as e:
        bot.send_message(message.chat.id, "*shi se add kro na *", parse_mode='@raj14754')
...
def attack_command(message):
    ...
    bot.send_message(message.chat.id, "*Please provide the target IP and port separated by a space.*", parse_mode='@raj14754')
...
def process_attack_ip_port(message):
    try:
        ...
        if len(args) != 2:
            bot.send_message(message.chat.id, "*Invalid format. Provide both target IP and port.*", parse_mode='@raj14754')
            return

        target_ip, target_port = args[0], int(args[1])
        if target_port in blocked_ports:
            bot.send_message(message.chat.id, f"*Port {target_port} is blocked. Use another port.*", parse_mode='@raj14754')
            return
...
def start_attack(message):
    ...
    if attack_details:
        ...
        bot.send_message(message.chat.id, f"*Attack started on Host: {target_ip} Port: {target_port}*", parse_mode='@raj14754')
    else:
        bot.send_message(message.chat.id, "*No target specified. Use /Attack to set it up.*", parse_mode='@raj14754')
...
def stop_attack(message):
    ...
    if attack_details:
        ...
        bot.send_message(message.chat.id, f"*Attack stopped on Host: {target_ip} Port: {target_port}*", parse_mode='@raj14754')
        user_attack_details.pop(message.from_user.id, None)
    else:
        bot.send_message(message.chat.id, "*No active attack found to stop.*", parse_mode='@raj14754')
